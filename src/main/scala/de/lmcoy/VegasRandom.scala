...
case class VegasRandom(ndim: Int, nbin: Int, xi: Matrix, allEvaluations: Evaluation, f: List[Double] => Double) {

  private case class FunctionArgs(wgt: Double, x: List[Double], ia: List[Int])

  private def functionArgs(random: Random): FunctionArgs = {
    @tailrec
  ...
  def iteration(n: Int)(implicit spark: SparkSession): VegasRandom = {
    ...
    new VegasRandom(ndim, nbin, nxi, allEvaluations + iteration, f)
  }

  private def it(n:Int)(implicit spark: SparkSession) : Evaluation = {
    val partitions = 4
...
object VegasRandom {
  def apply(ndim: Int, nbin: Int, f: List[Double] => Double): VegasRandom = {
    new VegasRandom(ndim, nbin, initGrid(ndim, nbin), Evaluation(0,0,0, Matrix(nbin,ndim)),f)
  }
  ...
  case class Evaluation(n: Int, f: Double, f2: Double, d: Matrix) {
    def +(other: Evaluation) : Evaluation = {
      Evaluation(n + other.n, f + other.f, f2 + other.f2, d.addMatrix(other.d))
    }
  }
